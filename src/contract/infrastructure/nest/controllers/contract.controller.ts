...
export class ContractController {
  ...
  async create(@Body() body: CreateContractDTO, @Req() req: IUserRequest) {
    ...
    const contractData = {
      ...
      extension: body.extension ? {
        ...body.extension,
        newEndDateTime: body.extension.newEndDateTime ? body.extension.newEndDateTime : undefined,
      } : undefined,
    };
  ...
  @ApiResponse({ status: 404, description: 'Contract not found' })
  @ApiBody({ type: UpdateContractDTO, description: 'Data to update a Contract' })
  @Roles(TypeRoles.ADMIN, TypeRoles.SELLER, TypeRoles.SUPERVISOR, TypeRoles.SUPERADMIN)
  @UseGuards(AuthGuards, RoleGuard)
  async update(
    @Param('id') id: string,
