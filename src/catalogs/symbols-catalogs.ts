const SymbolsCatalogs = {
  ICatRoleRepository: Symbol.for('ICatRoleRepository'),
  ICatRoleService: Symbol.for('ICatRoleService'),
  ICatDocumentRepository: Symbol.for('ICatDocumentRepository'),
  ICatDocumentService: Symbol.for('ICatDocumentService'),
  ICatCategoryRepository: Symbol.for('ICatCategoryRepository'),
  ICatCategoryService: Symbol.for('ICatCategoryService'),
  ICatPriceConditionRepository: Symbol.for('ICatPriceConditionRepository'),
  ICatPriceConditionService: Symbol.for('ICatPriceConditionService'),
  ICatPaymentMethodRepository: Symbol.for('ICatPaymentMethodRepository'),
  ICatPaymentMethodService: Symbol.for('ICatPaymentMethodService'),
  ICatCountryRepository: Symbol.for('ICatCountryRepository'),
  ICatCountryService: Symbol.for('ICatCountryService'),
  ICatModelRepository: Symbol.for('ICatModelRepository'),
  ICatModelService: Symbol.for('ICatModelService'),
  ICatStatusRepository: Symbol.for('ICatStatusRepository'),
  ICatStatusService: Symbol.for('ICatStatusService'),
  ICatContractEventRepository: Symbol.for('ICatContractEventRepository'),
  ICatContractEventService: Symbol.for('ICatContractEventService'),
};

export default SymbolsCatalogs;
